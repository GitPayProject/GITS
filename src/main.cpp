...
#if defined(NDEBUG)
#error "GITS cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uGITS) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 36) {
        nSubsidy = 4.5 * COIN;
    }
	else if (nHeight <= 3000 && nHeight > Params().LAST_POW_BLOCK()) {
        nSubsidy = 6.5 * COIN; 
    }
	else if (nHeight <= 10000 && nHeight >= 3001) {
        nSubsidy = 7.5 * COIN; 
    }
	else if (nHeight <= 26600 && nHeight >= 10001) {
        nSubsidy = 8 * COIN; 
    }
	else if (nHeight <= 51200 && nHeight >= 26001) {
        nSubsidy = 9.5 * COIN; 
    }
	else if (nHeight <= 71800 && nHeight >= 51201) {
        nSubsidy = 12.5 * COIN; 
    }
	else if (nHeight <= 126800 && nHeight >= 71801) {
        nSubsidy = 8.5 * COIN;
    }
	else if (nHeight <= 625400 && nHeight >= 126801) {
        nSubsidy = 7.5 * COIN; 
    }
    	else  if (nHeight > 625400) {
        nSubsidy = 3 * COIN;
        nSubsidy >>= ((nHeight + 210000) / 1050000);
    }
    return nSubsidy;
}
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // GITS: We use certain sporks during IBD, so check to see if they are
